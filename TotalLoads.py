...
bplot=ax.boxplot([x for x in GWNL.flatten() if np.isnan(x)==False],whis=[5,95], boxprops=dict(facecolor='green', alpha=0.25),widths=0.6,patch_artist=True, positions=[0],labels=[labels[0]],showfliers=False, vert=False)

bplot=ax.boxplot([x for x in NSWGW.flatten() if np.isnan(x)==False],whis=[5, 95], boxprops=dict(facecolor='green', alpha=0.25),widths=0.6, patch_artist=True, positions=[1],labels=[labels[1]],showfliers=False, vert=False) 

bplot=ax.boxplot(NO3SY.flatten(), widths=0.6,whis=[5, 95],boxprops=dict(facecolor='green', alpha=0.25), patch_artist=True, positions=[2],labels=[labels[2]],showfliers=False, vert=False)

bplot=ax.boxplot(NO3LATY.flatten(), widths=0.6,whis=[5, 95], boxprops=dict(facecolor='green', alpha=0.25),patch_artist=True, positions=[3],labels=[labels[3]],showfliers=False, vert=False) 
bplot=ax.grid(True,linestyle ='--', axis='x')
plt.xlabel('kgNO3/ha/year')
fig.savefig(DRR+'/Nutrient.png', dpi=300, bbox_inches='tight')
